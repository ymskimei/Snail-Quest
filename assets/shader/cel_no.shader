...
void fragment() {
	...
	ALBEDO = albedo_color.rgb * texture(texture_albedo, UV).rgb;
	ALPHA = albedo_color.a * texture(texture_albedo, UV).a;
}

void light() {
	float NdotL = dot(NORMAL, LIGHT);
	float is_lit = step(shade_threshold, NdotL);
	vec4 base = texture(texture_albedo, UV).rgba * albedo_color.rgba;
	vec4 shade = texture(texture_albedo, UV).rgba * shade_color.rgba;
	vec4 diffuse = base;
	if (use_diffuse) {
	...
	if (use_specular) {
		...
		diffuse += specular_color.rgba * specular_value;
	}
	if (use_rim) {
		...
		diffuse += rim_color.rgba * rim_value * is_lit;
	}
	if (use_light) {
