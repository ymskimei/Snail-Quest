...
func _input(event: InputEvent) -> void:
	if is_controlled():
		...
		if Input.is_action_pressed(Utility.input.stick_main_down) and !active:
			_activate_switch()
		elif Input.is_action_pressed(Utility.input.stick_main_up) and active:
			_deactivate_switch()
		elif Input.is_action_pressed(Utility.input.action_main):
			if is_instance_valid(Auto.prev_controlled):
				Auto.prev_controlled.set_controlled()
	else:
