...
func has_gamepad() -> bool:
	return Device.get_connected_joypads().size() > 0

func guess_device_name(i: int = 0) -> String:
	var connected_joypads = Device.get_connected_joypads()
...
func rumble_small(target_device: int = 0) -> void:
	Device.start_joy_vibration(target_device, 0.4, 0, 0.1)

func rumble_medium(target_device: int = 0) -> void:
	Device.start_joy_vibration(target_device, 0, 0.7, 0.1)

func rumble_large(target_device: int = 0) -> void:
	Device.start_joy_vibration(target_device, 0, 1, 0.1)

func start_rumble_small(target_device: int = 0) -> void:
	Device.start_joy_vibration(target_device, 0.4, 0, 0)

func start_rumble_medium(target_device: int = 0) -> void:
	Device.start_joy_vibration(target_device, 0, 0.7, 0)

func start_rumble_large(target_device: int = 0) -> void:
	Device.start_joy_vibration(target_device, 0, 1, 0)

func stop_rumble(target_device: int = 0) -> void:
	Device.stop_joy_vibration(target_device)
