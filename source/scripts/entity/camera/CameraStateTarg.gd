...
func physics_process(delta: float) -> int:
	if entity.target.target:
		Auto.utility.slerp_look_at(entity, entity.target.target.global_transform.origin, targeting_speed)
...
func _cam_shifting() -> Vector3:
	var rotation_x = Input.get_action_strength(Auto.input.stick_alt_right) - Input.get_action_strength(Auto.input.stick_alt_left)
	var rotation_y = Input.get_action_strength(Auto.input.stick_alt_up) - Input.get_action_strength(Auto.input.stick_alt_down)
	if is_inverted():
