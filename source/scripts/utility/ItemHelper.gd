...
func set_item(items: Array, item_index: int, item):
	var previous_item = items[item_index]
	items[item_index] = item
	emit_signal("items_updated", [item])
	return previous_item

func remove_item(items: Array, item_index: int):
	var previous_item = items[item_index]
	items[item_index] = null
	emit_signal("items_updated", [item_index])
	return previous_item

func add_item(items: Array, item, added_amount: int):
...
func switch_item(items: Array, item_index: int, target_index: int):
...
func replace_item(target_index: Array, item, new_item):
	var i = target_index.find(item)
	if(i != -1):
		target_index[i] = new_item
	emit_signal("items_updated", [new_item])

func duplicate_item_instances(items: Array):
	var duplicate_items = []
	for item in items:
