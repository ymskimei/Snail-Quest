...
func _ready() -> void:
	...
	SnailQuest.connect("controlled_health_change", self, "_on_controlled_health_changed")
...
#		match state:
#			"Orbi": 
#				var zoom_mode = SnailQuest.camera.states.current_state.zoom_mode
#				if zoom_mode:
#					cam_icon.texture = cam_zoom
#				else:
#					cam_icon.texture = cam_pan
#			"Targ":
#				var bars_active = SnailQuest.camera.states.current_state.bars_active
#				if bars_active:
#					if is_instance_valid(SnailQuest.controlled):
#						var target_found = SnailQuest.controlled.target
#						if target_found:
...
func display_vehicle_boost() -> void:
	if is_instance_valid(SnailQuest.controlled):
		if SnailQuest.controlled is VehicleBody:
			var remaining = SnailQuest.controlled.boost_remaining
