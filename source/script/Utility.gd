...
#func randi_only_matches(value: int, matcher: int):
#	var result = false
#	while !result:
#		var random_number = randi() % value
#		if random_number == matcher:
#			result = true
...
func tr_key_exists(key: String) -> bool:
	return tr(key) != key
...
func damage(host: Spatial, size: Vector3 = Vector3(1.0, 1.0, 1.0), amount: float = 1.0, impact_pause: float = 0.0) -> void:
	var damager: Area = load("res://source/scene/entity/damager.tscn").instance()
