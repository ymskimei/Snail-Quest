...
func enter() -> void:
	...
	tween_cam_zoom(zoom_targeting, distance_targeting)
	print("Camera State: LOCK")

func physics_process(delta):
	WorldMathHelper.slerp_look_at(camera, cam_target, targeting_speed)
	camera.translation = lerp(camera.translation, camera.camera_target.translation + targeting_offset, follow_speed * delta)
