...
func on_command_list(_console, _args: Array) -> void:
	var entities: PackedStringArray = Utility.get_names_from_paths(SB.scene["entity"], ".tscn")
	var objects: PackedStringArray = Utility.get_names_from_paths(SB.scene["object"], ".tscn")
	var items: PackedStringArray = Utility.get_names_from_paths(SB.scene["item"], ".tscn")
...
func on_command_warp(_console, args: Array) -> void:
	...
	if SB.controlled and SB.controlled is Entity and SB.world:
		...
		var directory = DirAccess.open(warp_path)
		if directory:
...
func on_command_spawn(_console, args: Array) -> void:
	...
	if path in all:
		...
		var new_pos: Vector3 = SB.camera.global_position
		var new_rot: Vector3 = SB.camera.global_rotation
		for i in args[1]:
			var entity = load(path).instantiate()
			room.add_child(entity)
			if SB.controlled:
				new_pos = SB.controlled.global_position
