...
func _set_strings() -> void:
	info["title"] = Utility.read_config(cfg, "plugin", "name")
	info["description"] = Utility.read_config(cfg, "plugin", "description")
	info["author"] = Utility.read_config(cfg, "plugin", "author")
	info["version"] = Utility.read_config(cfg, "plugin", "version")
	#Move this later
