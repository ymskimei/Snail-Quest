...
func set_entity_hat(next: bool = true):
	identity.mesh_hat = hats[Utility.array_cycle(hat.get_mesh(), hats, next)]
	identity.pattern_hat = hat_patterns[Utility.array_cycle(hat_mat.get_shader_param("texture_albedo"), hat_patterns, next)]
...
func set_entity_sticker(next: bool = true):
	identity.pattern_sticker = stickers[Utility.array_cycle(sticker_mat.get_shader_param("texture_albedo"), stickers, next)]
...
#		if shell_color_accent.is_equal_approx(shell_color):
#			shell_color_accent = shell_color_accent.inverted()
